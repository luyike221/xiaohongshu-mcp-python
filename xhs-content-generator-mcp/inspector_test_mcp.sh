...
# MCP Inspector 调试脚本
# 用于启动 MCP Inspector 连接到小红书内容生成 MCP 服务进行调试
...
DEFAULT_PORT="8004"
MCP_ENDPOINT=""  # FastMCP HTTP 传输使用根路径
...
${BLUE}示例:${NC}
    ./inspector_test_mcp.sh                          # 连接到默认服务器 (http://localhost:8004)
    ./inspector_test_mcp.sh --port 9000              # 连接到自定义端口
    ./inspector_test_mcp.sh --auto-start             # 自动启动服务器并连接
    ./inspector_test_mcp.sh --host 0.0.0.0 --port 8004  # 连接到指定主机和端口
...
start_server() {
    ...
    # 使用 run.sh 启动服务器
    if [ -f "./run.sh" ]; then
        ./run.sh "$port" > /tmp/xhs-content-generator-mcp-server.log 2>&1 &
        SERVER_PID=$!
        echo -e "${GREEN}服务器已启动（PID: ${SERVER_PID}）${NC}"
        echo -e "${BLUE}日志文件: /tmp/xhs-content-generator-mcp-server.log${NC}"
        ...
        while [ $attempt -lt $max_attempts ]; do
            if check_server "$host" "$port" "$MCP_ENDPOINT" > /dev/null 2>&1; then
        ...
        echo -e "${YELLOW}请检查日志: /tmp/xhs-content-generator-mcp-server.log${NC}"
...
echo -e "${GREEN}  小红书内容生成 MCP 服务${NC}"
echo -e "${GREEN}========================================${NC}"
echo -e "${BLUE}服务器地址: ${MCP_URL}${NC}"
echo ""

# 检查服务器状态
if ! check_server "$HOST" "$PORT" "$MCP_ENDPOINT"; then
    if [ "$AUTO_START" = true ]; then
        ...
        echo -e "${YELLOW}或者先运行: ./run.sh ${PORT}${NC}"
        exit 1
    fi
fi
